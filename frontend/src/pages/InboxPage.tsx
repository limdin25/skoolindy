  ...
  const handleContinueAutomation = async () => {
    ...
    try {
      ...
      if (res.status === "sent") {
        ...
        // Optimistic update: set continuedAt on the conversation
        setSelected((prev) => prev ? { ...prev, continuedAt: prev.continuedAt || now } : prev);
        setBackendConversations((prev) =>
          prev.map((c) => c.id === selected.id ? { ...c, continuedAt: c.continuedAt || now } : c)
        );
        await refresh();
      }
    } catch {}
                    ...
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="flex-1 min-w-0 overflow-hidden">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5 min-w-0 overflow-hidden">
                          ...
                          {(conv.aiDmCount ?? 0) > 0 && <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-500/10 text-blue-600 whitespace-nowrap flex-shrink-0" title={`${conv.aiDmCount} AI DM${(conv.aiDmCount ?? 0) > 1 ? 's' : ''} sent`}>{conv.aiDmCount} AI</span>}
                          {(conv.followUpCount ?? 0) > 0 && <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-orange-500/10 text-orange-600 whitespace-nowrap flex-shrink-0" title={`${conv.followUpCount} follow-up${(conv.followUpCount ?? 0) > 1 ? 's' : ''} sent`}>{conv.followUpCount} FU</span>}
                          {conv.continuedAt && <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-gray-500/10 text-gray-500 whitespace-nowrap flex-shrink-0" title={`Manually continued at ${conv.continuedAt}`}><Play className="w-2.5 h-2.5" />continue</span>}
                          {conv.followUpDueAt && (() => { const d = Math.max(0, Math.floor((new Date(conv.followUpDueAt).getTime() - Date.now()) / 1000)); if (d <= 0) return null; const h = Math.floor(d/3600); const m = Math.floor((d%3600)/60); return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-gray-500/10 text-gray-500 whitespace-nowrap flex-shrink-0" title="Next follow-up">{h > 0 ? `${h}h ${m}m` : m > 0 ? `${m}m` : '<1m'}</span>; })()}
                        </div>
