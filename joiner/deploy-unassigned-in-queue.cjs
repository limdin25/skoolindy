#!/usr/bin/env node
/**
 * Include unassigned communities in /api/queue when profile_id=all.
 * Run on server: cd /root/.openclaw/workspace/community-join-manager && node deploy-unassigned-in-queue.cjs
 */
const fs = require('fs');
...
if (s.includes(oldHandler)) {
  ...
  console.log('OK: /api/queue now includes unassigned when profile_id=all');
} else if (s.includes("showUnassigned = !profile_id")) {
  console.log('Already patched.');
} else {
  console.error('Could not find exact handler. Manual patch may be needed.');
  process.exit(1);
}
